...
const handleStatusChange = (index: number, newStatus: string, booking: BookingData) => {
  ...
  if (newStatus === 'cancelled') {
    ...
    bookings.value[globalIndex].status = newStatus as BookingData['status'];
  }
};
...
<template>
  <main class="py-5 lg:px-10 md:px-5 px-4 w-full">
    ...
    <!-- Booking Details -->
    <div class="bookings_container flex flex-col items-center justify-between min-h-[80vh]">
      ...
      <div v-else class="overflow-x-auto w-full">
        <table class="min-w-full text-left bg-white shadow-sm rounded-lg">
          ...
          <tbody>
            <tr v-for="(entry, index) in paginatedBookings" :key="index"
              ...
              <td class="whitespace-nowrap py-5 px-3 sm:px-4">
                <div class="relative inline-block w-[130px]">
                  <select :value="entry.status"
                    ...
                    :class="statusColors[entry.status]" aria-label="Booking status">
    ...
    <!-- Cancel Reason Modal -->
    <CancelReasonModal :booking="active_booking" :visible="showCancelModal" :reason="cancelReason"
